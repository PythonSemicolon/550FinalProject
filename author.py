...
def vectorize(source_dir, data, author, train, dev, is_positive):
...
def preprocess_data(source_dir):
    ...
    struct = {'x_train': [], 'y_train': [], 'x_dev': [], 'y_dev': []}
    ...
    for author in authors:
        ...
        (train, dev) = works[author]['positive']
        vectorize(source_dir, data, author, train, dev, is_positive=True) # Vectorize samples from all texts

        # For the negative works
        (train, dev) = works[author]['negative']
        vectorize(source_dir, data, author, train, dev, is_positive=False) # Vectorize samples from all texts
...
def main():
    ...
    authors_data = preprocess_data('data/authors/')

    # Save data to files
    with open('data/authors.json', 'w') as file:
        json.dump(authors_data, file, indent=2)

    # Save syllable fail count to file
    with open('data/syllable_fail_count_author.json', 'w') as file:
        file.write(str(total_syllable_fail_count[0]))
